...
import { Movie, Showtime, Booking, User, Theater, Showroom, Show } from '../types/database';
...
// Showrooms
export const showroomsCollection = firestore.collection(db, 'showrooms');
export const showroomService = {
  async getShowroomsByTheater(theaterId: string): Promise<Showroom[]> {
  console.log('Fetching showrooms from theater...');

  try {
    const docRef = firestore.doc(db, 'theaters', theaterId);
    const docSnap = await firestore.getDoc(docRef);

    if (!docSnap.exists()) {
      throw new Error('Theater does not exist on the system.');
    }

    const data = docSnap.data();
    const showroomIds = data.showrooms as string[];

    // Fetch all showroom documents in parallel
    const showroomDocs = await Promise.all(
      showroomIds.map(id =>
        firestore.getDoc(firestore.doc(db, 'showrooms', id))
      )
    );

    // Convert DocSnap → Showroom (your type)
    const showrooms: Showroom[] = showroomDocs.map(snap => {
      const srData = snap.data();
      if (!srData) {
        throw new Error(`Showroom ${snap.id} is missing or invalid`);
      }
      return srData as Showroom;
    });
...
// Theaters
export const theatersCollection = firestore.collection(db,'theaters');

export const theaterService = {
  async getAllTheaters(): Promise<Theater[]> {
    console.log('Fetching all theaters...');
    try {
      const snapshot = await firestore.getDocs(theatersCollection);
      return snapshot.docs.map(mapDoc<Theater>);
    } catch (error) {
      throw new Error(`getAllTheaters failed: ${error}`);
    }
  },

  async getTheaterShowtimesFromMovies(theaterId: string, movieId: string): Promise<string[]> {

  // 1. Get all showrooms inside the theater
  const showrooms = await showroomService.getShowroomsByTheater(theaterId);

  // 2. Get all show IDs across all showrooms
  const allShowIds = showrooms.flatMap(sr => sr.shows);

  if (allShowIds.length === 0) {
    return [];
  }

  // 3. Load all show documents in parallel
  const showDocs = await Promise.all(
    allShowIds.map(id =>
      firestore.getDoc(firestore.doc(db, "shows", id))
    )
  );

  // 4. Filter by movie AND extract formatted times
  const formattedTimes: string[] = showDocs
    .filter(doc => {
      const data = doc.data();
      return data && data.movie === movieId;
    })
    .map(doc => {
      const data = doc.data()!;
